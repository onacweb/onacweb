...
async function localApi(action, options={}){
  ...
  if(action==='login'){
    ...
    const u=list.find(x=>String(x.username).toLocaleLowerCase('tr-TR')===un&&x.password_hash===localHash(String(input.password||'')));
...
function updateModeBadge(){
  ...
  el.textContent=localMode?'YEREL MOD':'SUNUCU MODU';
  el.title=localMode?'Bu cihazdaki kayıtlar kullanılır. Ortak kullanım için PHP hosting üzerinden açın.':'Kullanıcılar sunucuda kalıcı ve ortak saklanır.';
}
async function api(action, options={}){
  if(localMode){updateModeBadge();return localApi(action,options)}
  try{
    const res=await fetch(`./api.php?action=${encodeURIComponent(action)}`,{
      credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json',...(options.headers||{})},...options
    });
    let data={};
    const raw=await res.text();
    try{data=raw?JSON.parse(raw):{}}catch{
      const clean=String(raw||'').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim().slice(0,220);
      throw new Error(clean?`PHP sunucu hatası: ${clean}`:'Sunucu boş/geçersiz cevap verdi. PHP desteğini ve api.php dosyasını kontrol edin.');
    }
    if(!res.ok||data.ok===false)throw new Error(data.error||'İşlem başarısız.');
    updateModeBadge();return data;
  }catch(err){
    if(err instanceof TypeError || /fetch|network/i.test(String(err?.message||''))){
      localMode=true;updateModeBadge();
